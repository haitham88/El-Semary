        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($orderBy && in_array($orderBy, $dataType->fields())) {
                ...
                if ($request->from_date && $request->to_date) {
                    ...
                    $dataTypeContent = $model::whereBetween('delivery_date', [$from_date, $to_date])->get();
                } else {
...
//        dd($request);
        $total_price = 0;
        $items_ids = $request->request->get("order_belongstomany_item_relationship");
        ...
        $request->request->set("total_price", $total_price);
        $dataType = Voyager::model('DataType')->where('slug', '=', $slug)->first();
...
//        dd($categories);
        ...
        return Voyager::view($view, compact('customers','categories', 'subCategories', 'dataType', 'dataTypeContent', 'isModelTranslatable'));
    }
    ...
    public function store(Request $request)
    {
        if ($request->request->get("first_name")){
            $latestCustomer = Customer::all()->sortByDesc("id")->first();
            if ($latestCustomer) {
                $customerSerial = $latestCustomer->serial_number + 1;
            } else {
                $customerSerial = 1670001;
            }
        ...
        $sub_categories = $request->request->get("sub_category");
        $specs = $request->request->get("specs");
        $prices = $request->request->get("price");
        $material_codes = $request->request->get("material_codes");
        $color_codes = $request->request->get("color_codes");
        $slug = $this->getSlug($request);
//        $timestamp = time();
//        $code = $timestamp + 100;
//        $code = "or" . (string)$code;
        $latestOrder = Order::all()->sortByDesc("id")->first();
        if ($latestOrder) {
            $orderSerial = $latestOrder->serial_number + 1;
        } else {
            $orderSerial = 1230001;
        }
        $latestItem = Item::all()->sortByDesc("id")->first();
        if ($latestItem) {
            $itemSerial = $latestItem->serial_number + 1;
        } else {
            $itemSerial = 1450001;
        }
        ...
        for ($i = 0; $i < count($categories); $i++) {
            if (count($orderItems) != 0) {
                $itemSerial = $orderItems[count($orderItems) - 1]["serial_number"] + 1;
            }
            $subCategoryName = "null";
            $subCategoryImages = null;
            $subCategory = SubCategory::with('processes')->where('id', $sub_categories[$i])->first();
            if ($subCategory){
            ...
            if ($sub_categories[$i] != 0){
                foreach($subCategory->processes as $process){
                    $itemOperation = [
                    ...
                    array_push($operations, $itemOperation);
        ...
        $data->employees()->attach($request->request->get("order_belongstomany_employee_relationship"));
